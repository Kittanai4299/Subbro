...
function searchsomething(elem) {
    ...
    var html = '';
    for (let i = 0; i < product.length; i++) {
        if (product[i].name.toLowerCase().includes(value)||product[i].name.toUpperCase().includes(value)) {
            html += `<div onclick="openProductDetail(${i})" class="product-items ${product[i].type}">
                    <img class="product-img" src="./img/${product[i].img}" alt="">
                    <p style="font-size: 1.2vw;">${product[i].name}</p>
...
function searchproduct(param) {
    ...
    else if (param == 'Allmeat') {
        $(".Allmeat").show();
    }
    else if (param == 'Bread') {
        $(".Bread").show();
    }
    else if (param == 'Vegetable') {
        $(".Vegetable").show();
    }
    else if (param == 'Sauce') {
        $(".Sauce").show();
...
function openProductDetail(index) {
    ...
    $("#mdd-img").attr('src','/test/img/' +product[index].img);
        ...
        for (let i = 0; i < cart.length; i++) {
            html += `<div class="cartlist-items">
                        <div class="cartlist-left">
                            <img src="/test/img/${cart[i].img}" alt="">
...
var buyButtonCounter = 0;
function buynow() {
    var totalPrice = calculateTotalPrice();
    countBuyButtonClicks();
    $.ajax({
        method: 'post',
        url: 'buynow.php',
        data: {
            ...
            if(response.RespCode == 200) {
                ...
                for (let i = 0; i < cart.length; i++) {
                    cartItemsHTML += `<p>
                                      <img src="/test/img/${cart[i].img}" alt=""style="width: 20px; height: 20px; ">
                                      ${i+1}. ${cart[i].name}(${cart[i].count})
                                      </p>`;
                }
                Swal.fire({
                    icon: 'success',
                    title: 'Thank you',
                    html: ` <h3>Order Id : ${buyButtonCounter}</h3>
                            <h3>Order list</h3>
                            ${cartItemsHTML}
                            <p> Amount : ${response.Amount.Amount} THB</p>
                            <img src="/test/img/QR.png" alt="" >
                            `
